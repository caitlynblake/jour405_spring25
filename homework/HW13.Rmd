...
The expected results seem like they are on par wih the actual results. therefore I do not thin the differences are meaningful.
...
```{r}
alpha <- 0.05
df <- length(grade_data$Grade) - REPLACE_ME
...
The chi-square statistic is greater than the critical value, therfore we reject the null hypothesis.
...
The notable discrepancies are grades A and C. They have the largest differences (+2 and -20).

Questions:
Were there any resources offered like tutoring?
Did the instructor curve grades and how would that have affected the data?
